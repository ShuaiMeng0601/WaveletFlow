...
def euler_sampler(
    ...
    """Integrate the conditional velocity field from noise to data.

    Classifier-free guidance is intentionally unavailable because the current
    training pipeline does not learn a null-condition branch.
    """
    if cfg_scale != 1.0:
        raise ValueError(
            "Classifier-free guidance is not trained; use cfg_scale=1.0."
        )
